...
from model.basic_model.resnet import ResNet18
...
class ExpressionEmbed(nn.Module):
    ...
    def forward(self, data_dict, use_aug = True):
        
        if 'masked_source_img' in data_dict:
            inputs_face = torch.cat([data_dict['masked_source_img'], data_dict['masked_target_img']])
        else:
        ...
        theta = torch.cat([data_dict['source_theta'][:, :3, :], data_dict['target_theta'][:, :3, :]]).detach()
        ...
        with torch.no_grad():
            ...
            if not self.cfg.use_smart_scale:
                ...
                scale[:, 2, 2] = 1
                inv_theta_2d = torch.bmm(inv_theta_2d, scale)[:, :2].detach()

                scale_t = copy.deepcopy(scale)
                scale_t[0, [0, 1], [0, 1]] = scale[-1, [0, 1], [0, 1]]
                scale_t[1, [0, 1], [0, 1]] = scale[-1, [0, 1], [0, 1]]
                inv_theta_targ_2d = torch.bmm(inv_theta_targ_2d, scale_t)[:, :2].detach()
                scale_full[:, [0, 1, 2], [0, 1, 2]] = 2
                inv_theta_targ_2d_full = torch.bmm(inv_theta_targ_2d, scale_full)[:, :2].detach()
            ...
            data_dict['align_warp'] = align_warp_targ.bmm(inv_theta_targ_2d.transpose(1, 2)).view(n + t, self.grid_size, self.grid_size, 2)

            align_warp_targ = self.identity_grid_512.repeat_interleave(n + t, dim=0)
            data_dict['align_warp_full'] = align_warp_targ.bmm(inv_theta_targ_2d_full.transpose(1, 2)).view(n + t, 512, 512, 2)

        exp_embed = self.net_face(inputs_face_aligned)[0]
        data_dict['source_exp_embed'] = exp_embed[:n]
        data_dict['target_exp_embed'] = exp_embed[-t:]

        return data_dict

    def estimate_theta(self, source_keypoints, target_keypoints):
        ...
        param = param.to(keypoints.device)
